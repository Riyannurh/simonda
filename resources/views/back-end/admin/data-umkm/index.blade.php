...
<div class="card">
    <div class="card-body py-2">
        <form method="GET" action="{{ route('admin.data-umkm.index') }}" id="formFilter">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2">
                    <label class="form-label mb-1 small">Pendata</label>
                    <select class="form-select form-select-sm" name="pendata" onchange="this.form.submit()">
                        <option value="">Semua Pendata</option>
                        @foreach($pendataList ?? [] as $p)
                        <option value="{{ $p->id }}" {{ request('pendata') == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-1 d-flex gap-1">
                    ...
                    <a href="{{ route('admin.data-umkm.index') }}" class="btn btn-outline-secondary btn-sm w-100" title="Reset">
...
@if(request()->hasAny(['search','kelas','kecamatan','tgl_dari','tgl_sampai','pendata']))
<script>
                ...
                <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
                    ...
                    <div class="step-content active" id="edit_step1">
                        ...
                        <div class="row g-3">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-outline-secondary" id="editBtnPrev" style="display: none;">
                                            <i class="ri-arrow-left-line me-1"></i> Sebelumnya
                                        </button>
                                        <button type="button" class="btn btn-primary" id="editBtnNext">
                                            Selanjutnya <i class="ri-arrow-right-line ms-1"></i>
                                        </button>
                                        <button type="submit" class="btn btn-primary" id="editBtnSubmit" style="display: none;">
...
<style>
/* Pastikan tombol yang di-hide benar-benar tidak tampil di modal-footer */
...
function apiWilayah(params) {
    ...
    return fetch('/admin/data-umkm/api/wilayah?' + key)
        .then(r => r.json())
        .then(data => {
...
function setSelectValue(selectEl, value) {
    ...
    let found = Array.from(selectEl.options).find(o => o.value === value);
    if (!found) found = Array.from(selectEl.options).find(o => o.value.toLowerCase() === value.toLowerCase());
    if (found) {
        selectEl.value = found.value;
        console.log('[setSelectValue]', selectEl.id, ':', value, ' kode:', found.dataset.kode);
        return found.dataset.kode || '';
    }
    console.warn('[setSelectValue] NOT FOUND:', selectEl.id, value);
    const fb = document.createElement('option');
    fb.value = value; fb.textContent = value;
    selectEl.appendChild(fb); selectEl.value = value;
    return '';
}
...
function updateProgress() {
    ...
    // Show/hide buttons (pastikan tidak pernah bersamaan)
    ...
    // Tambahan: pastikan tidak ada class d-inline atau show yang mengganggu
    btnNext.classList.remove('d-inline', 'show');
    btnSubmit.classList.remove('d-inline', 'show');
}
...
document.getElementById('btnNext').addEventListener('click', function() {
    if (currentStep < totalSteps) {
        if (currentStep === 1) {
            ...
            [nikEl, namaEl].forEach(el => {
                el.classList.remove('is-invalid');
                el.nextElementSibling?.classList.contains('invalid-feedback') && (el.nextElementSibling.textContent = '');
            });
...
document.getElementById('formCreate').addEventListener('submit', function(e) {
    ...
    fetch("{{ route('admin.data-umkm.store') }}", {
        method: 'POST',
        body: formData,
        headers: {
...
function showDetail(id) {
    ...
    fetch(`/admin/data-umkm/${id}/detail`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function updateEditProgress() {
    ...
    document.getElementById('editBtnPrev').style.display = editCurrentStep === 1 ? 'none' : 'inline-block';
    document.getElementById('editBtnNext').style.display = editCurrentStep === editTotalSteps ? 'none' : 'inline-block';
    document.getElementById('editBtnSubmit').style.display = editCurrentStep === editTotalSteps ? 'inline-block' : 'none';
}
...
async function fillEditWilayah(pemilik, usaha) {
    ...
    console.log('[Edit] Provinsi:', pemilik.provinsi_pemilik, '→ kode:', provinsiKode);
    ...
    if (provinsiKode) {
        ...
        console.log('[Edit] Kabupaten:', pemilik.kabupaten_pemilik, '→ kode:', kabupatenKode);
        ...
        if (kabupatenKode) {
            ...
            console.log('[Edit] Kecamatan:', pemilik.kecamatan_pemilik, '→ kode:', kecamatanKode);
        ...
        console.warn('[Edit] Provinsi tidak ditemukan di dropdown:', pemilik.provinsi_pemilik);
    }

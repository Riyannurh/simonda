...
class LaporanController extends Controller
{
    private function buildQuery(Request $request)
    {
        ...
        if ($request->pendamping)     $query->where('id_pendata', $request->pendamping);
    ...
    public function index(Request $request)
    {
        ...
        return view('back-end.kepala-bagian.laporan.index', compact(
    ...
    public function exportExcel(Request $request)
    {
        ...
        $pendataObj  = $request->pendamping ? User::find($request->pendamping) : null;
        $filename    = 'laporan-umkm-' . now()->format('Ymd-His') . '.xlsx';
        $spreadsheet = new Spreadsheet();

        $sheet1 = $spreadsheet->getActiveSheet()->setTitle('Semua Data');
        $this->fillSheet($spreadsheet->getActiveSheet(), $allData, $pendataObj ? $pendataObj->name : 'Semua Pendata');
        ...
        if (!$pendataObj) {
            ...
            foreach ($pendataList as $pendata) {
                ...
                if ($dataPendata->isEmpty()) continue;
    ...
    private function fillSheet($sheet, $data, string $pendataLabel): void
    {
        $titleStyle  = ['font' => ['bold' => true, 'size' => 13, 'color' => ['argb' => 'FF1F3864']], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]];
        $groupStyle  = ['font' => ['bold' => true, 'color' => ['argb' => 'FFFFFFFF'], 'size' => 9], 'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['argb' => 'FF34495E']], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER, 'vertical' => Alignment::VERTICAL_CENTER], 'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['argb' => 'FF888888']]]];
        $headerStyle = ['font' => ['bold' => true, 'color' => ['argb' => 'FFFFFFFF'], 'size' => 9], 'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['argb' => 'FF2C3E50']], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER, 'vertical' => Alignment::VERTICAL_CENTER, 'wrapText' => true], 'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['argb' => 'FF888888']]]];
        ...
        $sheet->setCellValue('A2', 'Pendata');   $sheet->setCellValue('B2', $pendataLabel);
        $sheet->setCellValue('A3', 'Total Data'); $sheet->setCellValue('B3', $data->count() . ' UMKM');
        $sheet->setCellValue('A4', 'Dicetak');    $sheet->setCellValue('B4', now()->format('d/m/Y H:i') . ' WIB');
        $sheet->getStyle('A2:A4')->getFont()->setBold(true);

        foreach (['A6:H6' => 'DATA PEMILIK', 'I6:R6' => 'DATA USAHA', 'S6:T6' => 'LEGALITAS', 'U6:V6' => 'SOSIAL MEDIA', 'W6:W6' => 'PENDATA'] as $range => $label) {
            [$start] = explode(':', $range);
        ...
        $columns = [['A',5],['B',18],['C',22],['D',13],['E',16],['F',14],['G',14],['H',35],['I',25],['J',16],['K',16],['L',14],['M',18],['N',18],['O',30],['P',10],['Q',18],['R',18],['S',20],['T',22],['U',18],['V',25],['W',22]];
        $headers = ['No','NIK','Nama Pemilik','Jenis Kelamin','Tempat Lahir','Tanggal Lahir','No. HP','Alamat Pemilik','Nama Usaha','Merek/Brand','Kategori','Kelas Usaha','Kecamatan Usaha','Desa Usaha','Alamat Usaha','Karyawan','Omset/Bulan (Rp)','Total Aset (Rp)','Jenis Legalitas','Nomor Legalitas','Platform Sosmed','URL/Username Sosmed','Pendata'];
        foreach ($columns as $i => [$col, $width]) {
        ...
        foreach ($data as $item) {
            $pemilik = $item->pemilik; $legalitas = $item->legalitas; $sosmed = $item->sosialMedia;
            ...
            for ($r = 0; $r < $maxRows; $r++) {
                ...
                $leg = $legalitas->get($r);
                if ($leg) { $sheet->setCellValue("S{$cur}", $leg->jenis ?? '-'); $sheet->setCellValue("T{$cur}", $leg->nomor ?? '-'); }
                $sm = $sosmed->get($r);
                if ($sm) { $sheet->setCellValue("U{$cur}", $sm->platform ?? '-'); $sheet->setCellValue("V{$cur}", $sm->url ?? '-'); }
                if ($no % 2 === 0) $sheet->getStyle("A{$cur}:W{$cur}")->applyFromArray($evenFill);
    ...
    public function exportPdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('back-end.kepala-bagian.laporan.pdf', compact('dataUmkm', 'pendata'))
